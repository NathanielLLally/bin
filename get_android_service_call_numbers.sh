#!/bin/bash
#
#  NOTE:  Requires adb connection, either over WIFI or ADB
#
#  TODO, check for adb connection and bail unless
#
ADBShell () {
  adb ${2+-s }$2 shell "$1" | tr -d '\r'
}
...
GetAndroidVersion () {
    ...
    ANDROID_SERIAL=${1:-$(ADBShell 'getprop ro.serialno')}
...
ParseServiceAIDL () {
    ...
    gcc -P -E - | tr '{};\n\r' '\n\n\n  ' | awk '$0~/\w/ { print $0 }' | sed -e '1,/interface\s/ d' | cat -n
}

AbortIfNoADB () {
  OUTBUF=$(adb ${2+-s }$2 shell 'getprop ro.serialno' 2>&1)
  [ "${OUTBUF}-" == "error: no devices/emulators found-" ] && echo "No ADB device connection available" && exit 0
}
